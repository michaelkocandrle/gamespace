...
| Hull sparks        | `USpaceHullSparksComponent` on the hull: streaks born on the ship (60 % on the front half) that pour back along it - 420 short blue sparks in a quantum jump, 60 faint white ones in flight from 30 m/s. Drawn with `M_SpaceDust` (own dynamic instance for the colour); hidden within 5-12 m of the cockpit eye |
