...
def good_manifest():
    _, c = gx.validate(good_ship(), METRIC)
    return gx.build_manifest(c, gx.plan_exports(c), "Vanguard.blend")
...
class ManifestValidationTest(unittest.TestCase):
    ...
    def test_consistency(self):
        ...
        manifest["meshes"]["SM_Ship_Vanguard_LOD1"]["lod"] = 2
    ...
    def test_files_and_meshes_must_match(self):
        manifest = good_manifest()
        manifest["files"] = [f for f in manifest["files"] if f["fbx"] != "SM_Ship_Vanguard_Canopy.fbx"]
        manifest["files"].append({"fbx": "../evil.fbx", "objects": ["SM_Ship_Vanguard_Wing"]})
        text = "\n".join(errors(gx.validate_manifest(manifest)))
